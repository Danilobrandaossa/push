...
class Gteck_Push {
    ...
    public function add_config_script() {
        ...
        $expected_key = 'BIJfFcoBwqS1RLu7tjMcdwIQK86T4KdRHhc6mcxFmy0yXp0DeNY8lRl0LSFp4XThozLwobq09dzEOOcSPwstI7k';
        $keys_match = ($vapid_public_key === $expected_key);
        
        // Log VAPID key info (full key for debugging)
        $this->log('Configurando VAPID public key no frontend', 'info', array(
            'vapid_key_full' => $vapid_public_key, // Log full key for comparison
        ...
        if (!$keys_match) {
            $this->log('ATENÇÃO: Chave VAPID não corresponde à chave esperada!', 'warning', array(
                'expected_key' => $expected_key,
                'actual_key' => $vapid_public_key,
                'keys_match' => false
            ));
        }
