...
class WebPushProvider {
  ...
  private async generateVapidHeaders(audience: string): Promise<Record<string, string>> {
    try {
      ...
      // Check if key appears to be in encrypted format (iv:tag:encrypted with 3 parts)
      // If so, this is an error - the key should have been decrypted before reaching here
      if (cleanPrivateKey.split(':').length === 3) {
        const parts = cleanPrivateKey.split(':')
        if (parts[0]?.length === 32 && parts[1]?.length === 32) {
          // This looks like encrypted format - should not be used directly
          throw new Error(
            'VAPID private key appears to be in encrypted format but was not decrypted. ' +
            'This usually means ENCRYPTION_KEY is missing or incorrect. ' +
            'Please set ENCRYPTION_KEY in your environment or re-configure VAPID keys.'
          )
        }
      }

      // Detect format: base64url uses - and _ (VAPID keys from generateVapidKeys are always base64url)
      // Standard base64 uses + and / instead
      let privateKeyBuffer: Buffer
      if (cleanPrivateKey.includes('-') || cleanPrivateKey.includes('_')) {
        // base64url format (standard for VAPID keys generated by generateVapidKeys)
        console.log('[WebPush] Using base64url format for private key')
        try {
          privateKeyBuffer = Buffer.from(cleanPrivateKey, 'base64url')
        } catch (error) {
          throw new Error(`Failed to decode VAPID private key as base64url: ${error instanceof Error ? error.message : 'Unknown error'}. Key length: ${cleanPrivateKey.length} chars. Expected ~43 chars for raw key or ~160 chars for PKCS#8 DER.`)
        }
      }
      else if (cleanPrivateKey.includes('+') || cleanPrivateKey.includes('/')) {
        ...
        try {
          ...
          throw new Error(`Failed to decode VAPID private key as base64: ${error instanceof Error ? error.message : 'Unknown error'}`)
        }
      }
      else {
        // Try base64url first (default for VAPID keys)
        console.log('[WebPush] Defaulting to base64url format for private key')
        try {
          privateKeyBuffer = Buffer.from(cleanPrivateKey, 'base64url')
          // Verify it decodes correctly by checking length
          // Raw EC private key: 32 bytes (43 chars base64url)
          // PKCS#8 DER for EC prime256v1: ~118 bytes (~160 chars base64url)
          if (privateKeyBuffer.length !== 32 && (privateKeyBuffer.length < 100 || privateKeyBuffer.length > 200)) {
            throw new Error(`Invalid key length: ${privateKeyBuffer.length} bytes. Expected 32 (raw) or ~118 (PKCS#8 DER)`)
          }
        } catch (error) {
          // Fallback to base64 if base64url fails
          console.log('[WebPush] base64url decode failed, trying base64 format')
          try {
            privateKeyBuffer = Buffer.from(cleanPrivateKey, 'base64')
            if (privateKeyBuffer.length !== 32 && (privateKeyBuffer.length < 100 || privateKeyBuffer.length > 200)) {
              throw new Error(`Invalid key length after base64 decode: ${privateKeyBuffer.length} bytes`)
            }
          } catch (base64Error) {
            throw new Error(
              `Failed to decode VAPID private key. Tried base64url and base64. ` +
              `Key length: ${cleanPrivateKey.length} chars. ` +
              `Error: ${base64Error instanceof Error ? base64Error.message : 'Unknown error'}. ` +
              `The key may be in encrypted format or corrupted.`
            )
          }
        }
      }
