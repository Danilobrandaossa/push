  ...
  private async generateVapidHeaders(audience: string): Promise<Record<string, string>> {
    try {
      ...
      if (cleanPrivateKey.includes('-') || cleanPrivateKey.includes('_')) {
        ...
        try {
          privateKeyBuffer = Buffer.from(cleanPrivateKey, 'base64url')
          // Verify it decodes correctly by checking length (PKCS#8 DER for EC prime256v1 is ~118 bytes)
          if (privateKeyBuffer.length < 100 || privateKeyBuffer.length > 200) {
            throw new Error('Invalid key length for base64url, trying base64')
          }
        } catch {
